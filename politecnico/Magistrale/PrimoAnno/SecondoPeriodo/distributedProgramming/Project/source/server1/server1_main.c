...
int myService(int s){
   ...
   do{  
       ...
       if (n < 0)
	{
	       ...
	       //sendErr(s);
	      // continue;// i try to send another file to the same client
	      return ERROR;
	}
	else if (n==0)
	 {
	 ...
	 /*i try to read the filename sent by the client*/
           ...
           if(res==ERROR){ 
              printf("Error occured while sending the file\n");
               // sendErr(s);
	           //continue;
	         return ERROR;
           }
...
int getFileName(int s,char *filename){
     ...
     //i should find a way to handle the case where the 2 last carachters are \r \n
            do {
                ...
                if( nread != 1){
                    if(nread==0){return 0;}//connection closed by party
                    else{ return ERROR;}
